...
const Home = ({
  ...
  const handleNewConversation = async (appendVal:any = {}) => {
    const lastConversation = conversations[conversations.length - 1];
    const oldConversation = JSON.parse(await localStorage.getItem('conversationHistory') || '[]');
    ...
    const updatedConversations = [...oldConversation, newConversation];
  ...
  useEffect(() => {
    ...
    if(paramFid && paramFid != '') {
      if (conversationHistory) {
          ...
          const currentConversationList = parsedConversationHistory.filter((item:Conversation) => item?.fileInfo?.fileid === paramFid);
          if(currentConversationList && currentConversationList.length > 0) {
            const currentConversation = currentConversationList[0];
            dispatch({
              field: 'selectedConversation',
